...
# [6] outputTransformed - Binary value; If False, outputs a table and graph based on the adf results.
#                         If true, outputs a data.table object with the data after different
#                         transformations.
# Outputs:
...
identifyDifferencing <- function( dataSeries, dLow, dHigh,size = 20,steps = 20,threshold = 1e-5, pThreshold = .1,outputTransformed = FALSE){ 
  ...
  if(outputTransformed==TRUE){
    returnSeries <- data.frame(diffSeries)
    names(returnSeries) <- dRange
    return(returnSeries)
  }else{
...
identifyDifferencingNew <- function( dataSeries, dLow, dHigh,size = 20,steps = 20,threshold = 1e-5, pThreshold = .1,outputTransformed = FALSE){ 
  dStep = (dHigh-dLow)/(steps )
  dLow <- ifelse(dLow ==0,  dStep,dLow)
  dRange <- seq(dLow,dHigh,dStep)
  # Generate a list of transformed data series as a list.
